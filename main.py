...
for data in [train, test]:
    ...
    X_sim=evaluate(model,X0,nx,data["u"],dt)
...
ax2[0].plot(train["y_sim"].squeeze().tolist(), 'b', label='y (sim)')
ax2[0].plot(train["y"], 'k', label='y (norm)') # why here not train_raw? TODO
ax2[1].set_title("test data, RMSe: {:.2}".format(test["error"]), color=color)
ax2[1].plot(test["y_sim"].squeeze().tolist(), 'b', label='y (sim)')
ax2[1].plot(test["y"], 'k', label='y (norm)')  # why here not test raw ? TODO
for ax in ax2:
    ax.legend()
    ax.grid()
    ax.tick_params(colors=color)
